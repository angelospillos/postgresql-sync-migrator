...
function parseSchedule(schedule, now) {
    ...
    let matchMonth = parseValues(month.split(','), 1, now.month);
    if (!matchMonth) {
        const monthValues = month.split('-');
        if (monthValues.length > 1) {
            const [min, max] = monthValues.map(value => parseMonth(now.year, value));
            if (now.month >= min && now.month <= max) {
                const rangeValues = Array.from({ length: max - min + 1 }, (_, i) => min + i);
                if (rangeValues.includes(now.month)) matchMonth = true;
            }
        } else {
            const monthValue = parseMonth(now.year, month);
            if (monthValue !== null && monthValue === now.month) matchMonth = true;
        }
    }
    ...
    if (!matchDayOfWeek) {
        ...
        if (dayOfWeekValues.length > 1) {
            const [min, max] = dayOfWeekValues.map(parseDayOfWeek);
            const rangeValues = getRangeValues(min, max);
            matchDayOfWeek = rangeValues.includes(now.weekday);
        } else {
    ...
    return matchMonth && matchDayOfWeek;
}
